...
export const LyricVideo: React.FC<LyricVideoProps> = ({
  ...
  // Compute line fill progress for color animation
  ...
  function getLineColor(lineIdx: number, isActive: boolean, isPast: boolean): React.CSSProperties {
    if (!anim?.enabled) {
      return { color: isActive ? cfg.lyrics.activeColor : "#ffffff" };
    }
    if (isPast) {
    ...
    <AbsoluteFill style={{ backgroundColor: cfg.background.type === "solid" ? cfg.background.solidColor : "#0a0a0f" }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          fontWeight: cfg.title.fontWeight, fontFamily: cfg.title.fontFamily,
      ...
      <div style={{
        ...
        {lrcLines.map((line, i) => {
          ...
          const lineOpacity = isActive
            ? interpolate(currentTime, [line.time, line.time + cfg.lyrics.scrollSpeed * 0.7], [0.5, 1],
                { extrapolateLeft: "clamp", extrapolateRight: "clamp" })
            : isPast ? cfg.lyrics.inactiveOpacity : cfg.lyrics.futureOpacity;

          const lineScale = isActive
            ? interpolate(currentTime, [line.time, line.time + cfg.lyrics.scrollSpeed], [0.95, 1],
                { extrapolateLeft: "clamp", extrapolateRight: "clamp" })
            : 1;
          ...
          return (
            <div key={i} style={{
              ...
              opacity: lineOpacity, fontFamily: cfg.lyrics.fontFamily,
